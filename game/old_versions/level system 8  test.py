...
class Assets2:
    ...
    def Mushroom(self):
        ...
        if not self.spikes_generated:
            for _ in range(20):  # Number of spikes you want to generate
                while True:
                    x = random.randint(50, WIDTH - 50)
                    y = random.randint(50, HEIGHT - 50)

                    # Check that the spike is not too close to any mushroom or the player
                    too_close = False
                    for m in self.mushrooms:
                    ...
                    if not too_close:
                        player_x, player_y = self.player.centerx, self.player.centery
                        dist_to_player = math.sqrt((x - player_x) ** 2 + (y - player_y) ** 2)
                        if dist_to_player < self.safe_distance:  # Avoid spikes near the player
                            too_close = True
        ...
        for s in self.spike:
            pygame.draw.polygon(win, BLACK, s)  # Draw the spike as a polygon

            # Calculate bounding rectangle for the triangle (spike)
            min_x = min(p[0] for p in s)
            max_x = max(p[0] for p in s)
            min_y = min(p[1] for p in s)
            max_y = max(p[1] for p in s)

            spike_rect = pygame.Rect(min_x, min_y, max_x - min_x, max_y - min_y)

            # Append the spike bounding rectangle to spike_rects
            self.spike_rects.append(spike_rect)

        return self.mushrooms, self.spike
...
def Over():
    ...
    while running:
        ...
        press_key = font_medium.render("Press any key to try again", True, BLACK)
...
def Game2():
    ...
    while running:
        ...
        for s, rect in zip(spikes, assets.spike_rects):  # Correct access to spike_rects
            if player.colliderect(rect):  # Check collision with bounding rect
                Over()

        # Level completion condition (all mushrooms collected)
        if len(mushrooms) == 0:
            running = False
            Next_level(2)
