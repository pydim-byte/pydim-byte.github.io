...
LEVELS = {
    ...
    2: {"level_color": GREEN2, "with_spikes": True, "with_bad_mushrooms": False, "with_enemy": False},
    3: {"level_color": GREEN3, "with_spikes": False, "with_bad_mushrooms": True, "with_enemy": False},
    4: {"level_color": GREEN4, "with_spikes": False, "with_bad_mushrooms": False, "with_enemy": True},
}
...
class Assets:
    ...
    def Control(self):
        ...
        # If the player is holding down a movement key, increase the speed gradually
        if keys[pygame.K_LEFT] or keys[pygame.K_RIGHT] or keys[pygame.K_UP] or keys[pygame.K_DOWN]:
            if self.player_speed < self.max_speed:
                # Gradually increase speed to max speed, based on how long the key is held
                self.player_speed += self.acceleration_rate
                if self.player_speed > self.max_speed:
                    self.player_speed = self.max_speed  # Cap the speed at max
        else:
            # Reset speed to minimum if no key is pressed
        ...
        # Calculate the diagonal speed (normalize direction)
        ...
        # Apply movement based on normalized directions
        player.x += move_x * self.player_speed
        player.y += move_y * self.player_speed

        # Boundary check: prevent moving above the upper 40px border
        if player.x < 0:
            player.x = 0
        if player.x > WIDTH - player_size:
            player.x = WIDTH - player_size
        if player.y < 40:  # Prevent player from going above the 40px border
            player.y = 40
